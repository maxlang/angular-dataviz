...
angular.module('dataviz.directives').directive('aScatter', ['$timeout', 'VizUtils', function($timeout, VizUtils) {
  return {
    restrict: 'E',
    scope: {
      data: '=',
    ...
    link: function(scope, element) {
      ...
      var width, height, margins, range, max, leftMargin, w, h, bars, barPadding, barWidth, radius, svg, g, x, y, d, xAxis, yAxis, xAxisG, yAxisG, points, brush, brushRect;
...
//
        ...
        points = points.data(scope.data, key);
...
//      });
      ...
      function setSelected(filter, extent) {
        if (!extent || _.isEmpty(extent) || extent[0][0] === extent[1][0] || extent[0][1] == extent[1][1]) {
          scope.$apply(function () {
            filter[0].splice(0, filter[0].length);
            filter[1].splice(0, filter[1].length);
          });
        } else {
          scope.$apply(function () {
            filter[0].splice(0, filter[0].length, [extent[0][0], extent[1][0]]);
            filter[1].splice(0, filter[1].length, [extent[0][1], extent[1][1]]);
          });
        }
      }
      ...
      scope.$watch('params.filter', function(f) {
        if (f) {
          console.log('setting brush');
          var fclone = _.cloneDeep(f);
          setBrush(scope.brush, [[fclone[0][0][0],fclone[1][0][0]],[fclone[0][0][1],fclone[1][0][1]]]);
        }
      }, true);
